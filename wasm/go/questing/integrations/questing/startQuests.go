...
func StartQuests(this js.Value, args []js.Value) interface{} {
	...
	handler := js.FuncOf(func(this js.Value, args []js.Value) interface{} {
		...
		go func() {
			...
			enrollmentJson, err := startQuests(holder, oracle, questeeMints, uint64(questIndex))
			if err != nil {
				errorConstructor := js.Global().Get("Error")
...
func startQuests(holder, oracle solana.PublicKey, pixelBallzMints []string, questIndex uint64) ([]byte, error) {
	instructions := make([]solana.Instruction, 0)
	txJson := []byte("{}")

	for _, pixelBallzMint := range pixelBallzMints {
		fmt.Println(pixelBallzMint)
		if startQuestIx := startQuest(holder, oracle, solana.MustPublicKeyFromBase58(pixelBallzMint), questIndex); startQuestIx != nil {
...
func startQuest(holder, oracle, pixelBallzMint solana.PublicKey, questIndex uint64) *questing.Instruction {
	fmt.Println(oracle, holder, pixelBallzMint, questIndex)
	return ops.StartQuest(oracle, holder, pixelBallzMint, questIndex)
}
