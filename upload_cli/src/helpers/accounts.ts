...
export const createCandyMachineV2 = async function (
    ...
    const batches = await getBatches(payerWallet.publicKey);
    const batchesAccount = await anchorProgram.account.batches.fetch(batches);
    const batchReceipt = await getBatchReceipt(payerWallet.publicKey, batchesAccount.counter)


    // treasuryAuthority, candyMachineCreator
    const treasuryAuthority = await getTreasuryAuthority(payerWallet.publicKey);
    const [candyMachineCreator,] = await getCandyMachineCreator(candyAccount.publicKey);
    console.log(candyAccount.publicKey, candyMachineCreator)

    const whitelisted = await getTreasuryWhitelist(payerWallet.publicKey, treasuryAuthority, candyMachineCreator);
    const wlAddSig = await anchorProgram.rpc.addWhitelistedCm(candyMachineCreator, candyAccount.publicKey, {
        accounts: {
            treasuryWhitelist: whitelisted,
            oracle: payerWallet.publicKey,
            treasuryAuthority,
            systemProgram: SystemProgram.programId,
        },
        signers: [payerWallet],
    });
    console.log("--------whitelist add sig----------", wlAddSig);
    return {
        candyMachine: candyAccount.publicKey,
        uuid: candyData.uuid,
        txId: await anchorProgram.rpc.initializeCandyMachine(candyData, "riptard1", {
            accounts: {
                batchAccount: candyAccount.publicKey,
                oracle: payerWallet.publicKey,
                batches,
                batchReceipt,
                systemProgram: SystemProgram.programId,
...
export const getCandyMachineCreator = async (
    ...
    return await anchor.web3.PublicKey.findProgramAddress(
        [Buffer.from('someplace'), candyMachine.toBuffer()],
        CANDY_MACHINE_PROGRAM_V2_ID,
    );
};
...
export const getTreasuryAuthority = async function (
    oracle: PublicKey,
) {
    return (
        await PublicKey.findProgramAddress(
            [
                Buffer.from('someplace'),
                Buffer.from('ballz'),
                oracle.toBuffer(), 
            ],
            CANDY_MACHINE_PROGRAM_V2_ID,
...
export const getTreasuryWhitelist = async function (
    oracle: PublicKey,
    ...
    return (
        await PublicKey.findProgramAddress(
            [
                oracle.toBuffer(), 
                Buffer.from('treasury_whitelist'),
...
export const getBatches = async function (
    oracle: PublicKey,
) {
    return (
        await PublicKey.findProgramAddress(
...
export const getBatchReceipt = async function (
    oracle: PublicKey,
    index: anchor.BN,
) {
    return (
        await PublicKey.findProgramAddress(
            [oracle.toBuffer(), index.toBuffer('le', 8)],
            CANDY_MACHINE_PROGRAM_V2_ID,
        )
    )[0];
};
