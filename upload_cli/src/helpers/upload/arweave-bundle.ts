...
export async function* makeArweaveBundleUploadGenerator(
    ...
    if (storageType === StorageType.ArweaveSol) {
        ...
        const bufferCost = cost.multipliedBy(5).dividedToIntegerBy(2);
        log.info(
        ...
        if (currentBalance.lt(bufferCost)) {
            log.info(
                `Current balance ${currentBalance.toNumber()}. Sending fund txn...`,
            );
            try {
            ...
            log.info(
                `Current balance ${currentBalance.toNumber() / LAMPORTS
                } is sufficient.`,
            );
        }
    }
    ...
    if (balance.minus(5000).lte(0)) {
        ...
        try {
            ...
            if (withdrawResponse.status == 200) {
                log.info(
                    `Successfully withdrew ${withdrawResponse.data.final / LAMPORTS
                    } SOL.`,
                );
